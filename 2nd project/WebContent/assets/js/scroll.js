...
(function($) {
	...
	var $window = $(window), $body = $('body'), $wrapper = $('div#wrapper'), $main = $('#main2'), settings = {
		...
		scrollWheel : {
			...
			// Sets the scroll wheel factor. (Ideally) a value between 0 and 1
		...
		scrollZones : {
			...
			// Sets the speed at which the page scrolls when a scroll zone is
			// active (higher = faster scroll, lower = slower scroll).
			speed : 15
		...
		if (settings.keyboardShortcuts.enabled)
			(function() {
				...
				.on('keydown', function(event) {
					...
					// Page Up.
					case 33:
						$main.scrollLeft($main.scrollLeft() - $window.width()
								+ 100);
						scrolled = true;
						break;
		...
		if (settings.scrollZones.enabled)
			(function() {
				...
				var $left = $('<div class="scrollZone left"></div>'), $right = $('<div class="scrollZone right"></div>'), $zones = $left
						.add($right), paused = false, intervalId = null, direction, activate = function(
						d) {
				...
				$zones.appendTo($wrapper).on('mouseleave mousedown',
						function(event) {
							deactivate();
						});
...
(function($) {
	...
	var $window = $(window), $body = $('body'), $wrapper = $('div#wrapper0'), $main = $('#main0'), settings = {
